...
# Ensure OPENAI_API_KEY is set in your environment for OpenAI provider

def send_to_openai(image_path: str, model: str) -> str:
    """
    Sends an image to OpenAI's Chat API for zero-shot sign language recognition.
    ...
    try:
        response = openai.ChatCompletion.create(
            model=model,
            messages=[
                {"role": "user", "content": "Identify the meaning of this sign language image."}
            ],
            files=[{"name": os.path.basename(image_path), "data": img_bytes}]
        )
...
def send_to_ollama(image_path: str, model: str) -> str:
    """
    Sends an image to a running Ollama instance for zero-shot sign language recognition.
    Returns the model's prediction as text.
    """
    with open(image_path, "rb") as f:
        b64_str = base64.b64encode(f.read()).decode('utf-8')
    ...
    try:
        resp = requests.post(
            ...
            json={"model": model, "prompt": "Identify the meaning of this sign language image."}
        )
